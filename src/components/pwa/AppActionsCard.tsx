...
export default function AppActionsCard({
  ...
  const reminderSlots = useMemo(() => {
    const supplementSlots = getSupplementReminderSlots(dateKey, supplements, fastingContext);
    if (!digestInfo) return supplementSlots;
    const digestSlot = getDailyDigestSlot(dateKey, digestInfo);
    // Prepend digest — it fires at 07:00, before any supplement slot
    ...
    <div className="card p-4">
      ...
      {notificationState === "denied" && isReminderExpanded && (
        <div className="mt-2 rounded-xl p-2.5 text-[11px] leading-relaxed"
          ...
          Izin notifikasi diblokir — buka pengaturan site di browser dan ubah ke <strong>Allow</strong>.
        </div>
      )}
    </div>
  );
